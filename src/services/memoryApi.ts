...
const req=async<T>(path:string,s:Session,init:RequestInit={}):Promise<T>=>{const r=await serviceFetch(url()+path,{...init,headers:{'Content-Type':'application/json',Authorization:`Bearer ${s.accessToken}`,...init.headers}});if(!r.ok)throw new Error(safeStatusMessage(r.status,'Memory request failed.'));return r.status===204?undefined as T:r.json()}
