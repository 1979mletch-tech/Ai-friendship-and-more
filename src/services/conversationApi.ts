...
const call=async<T>(path:string,s:Session,init:RequestInit={}):Promise<T>=>{const r=await serviceFetch(base()+path,{...init,headers:{...headers(s),...init.headers}});if(!r.ok)throw new Error(r.status===401?'Your session has expired. Please sign in again.':'Conversation request failed.');return r.status===204?undefined as T:r.json()}
