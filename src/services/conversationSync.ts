...
export const loadRemoteConversations=async(s:Session):Promise<LocalConversation[]>=>{
 ...
 const full=await Promise.all(rows.map(async row=>({id:row.id,title:row.title,createdAt:row.updatedAt,updatedAt:row.updatedAt,messages:(await conversationApi.messages(s,row.id)).map(m=>({id:m.id,role:m.role,text:m.text,createdAt:m.createdAt,dayKey:m.createdAt.slice(0,10)}))})))
