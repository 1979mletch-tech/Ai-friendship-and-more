...
const call=async<T>(path:string,s:Session,init:RequestInit={})=>{const r=await serviceFetch(base()+path,{...init,headers:{Authorization:`Bearer ${s.accessToken}`,...init.headers}});if(!r.ok)throw new Error(safeStatusMessage(r.status,'Privacy request failed.'));return r.status===204?undefined as T:r.json() as Promise<T>}
